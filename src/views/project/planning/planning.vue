...
import {
  ...
  delPlanVipProject,
  addPlanVipProject,
  changePlanVipProject,
} from "@/api/api.js";
import planSearchMenu from "@/components/project/planSearchMenu";
// 按钮权限
import permission from "@/directives/permission";

export default {
  name: "planing",
  components: { planSearchMenu },
  // 自定义指令
  ...
  methods: {
    ...
    getMoreProjectData() {
      this.loading = true;
      getPlanVipProject(this.$store.state.vipProject.selectValue).then((res) => {
        ...
        this.lists.forEach((item) => {
          ...
          item.projectType === "1"
            ? (item.projectType_text = "住宅规划")
            : item.projectType === "2"
            ? (item.projectType_text = "文旅城规划")
            : item.projectType === "3"
            ? (item.projectType_text = "商业街规划")
            : item.projectType === "4"
            ? (item.projectType_text = "学校规划")
            : (item.projectType_text = "公园规划")
          item.address === "1"
    ...
    getProjectData(val) {
      this.end = false;
      this.loading = true;
      getPlanVipProject(val).then((res) => {
    ...
    addProjectData(val) {
      this.end = false;
      this.loading = true;
      addPlanVipProject(val).then((res) => {
    ...
    changeProjectData(val) {
      this.end = false;
      this.loading = true;
      changePlanVipProject(val).then((res) => {
        this.lists = res.data.data;
        this.lists.forEach((item) => {
          // 不要修改原数据
          item.department === "1"
    ...
    // 删除项目
    deleteProject(val) {
      this.$confirm("确认删除项目吗?").then(() => {
        delPlanVipProject(val).then((res) => {
          this.lists = res.data.data;
          this.lists.forEach((item) => {
            // 不要修改原数据
            item.department === "1"
